...
func gasSStore(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	// 	2.2. If original value does not equal current value (this storage slot is dirty), 200 gas is deducted. TransitionState both of the following clauses.
...
func gasKeccak256(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	if gas, overflow = math.SafeAdd(gas, Keccak256WordGas); overflow {
		return 0, errGasUintOverflow
	}
...
func gasExtCodeCopy(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	var overflow bool
	if gas, overflow = math.SafeAdd(gas, evm.gas_table.ExtcodeCopy); overflow {
		return 0, errGasUintOverflow
	}
...
func gasTLoad(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	return evm.gas_table.WarmStorageReadCost, nil
}
...
func gasExp(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	var (
		gas      = expByteLen * evm.gas_table.ExpByte // no overflow check required. Max is 256 * ExpByte gas
		overflow bool
	)
...
func gasCall(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	evm.call_gas_tmp, err = callGas(evm.gas_table, contract.Gas, gas, stack.Back(0))
	if err != nil {
		return 0, err
	}
	if gas, overflow = math.SafeAdd(gas, evm.call_gas_tmp); overflow {
		return 0, errGasUintOverflow
	}
	return gas, nil
}
...
func gasDelegateCall(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	var overflow bool
	if gas, overflow = math.SafeAdd(gas, evm.gas_table.Calls); overflow {
		return 0, errGasUintOverflow
	}
