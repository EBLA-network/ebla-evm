...
//			b) replace " by \" and copy&paste the ABI string into the EblaDposClientMetaData
//		 2. To get implementation bytecode(DposImplBytecode)
//			a) run `solc --bin-runtime --overwrite --optimize dpos_contract_impl.sol --output-dir .`
//			b) Copy bytecode from `DposDummyImpl.bin-runtime` file to `var DposImplBytecode` variable in `dpos_contract_solidity_structs.go` file.
//		 2. To generate solidity interface related structs:
//		 	a) run `abigen --abi=DposInterface.abi --pkg=eblaDposClient --out=dpos_contract_interface.go`
...
type CancelUndelegateArgs struct {
	Validator      common.Address
	UndelegationId uint64
}
...
type GetUndelegationArgs struct {
	Delegator      common.Address
	Validator      common.Address
	UndelegationId uint64
}
