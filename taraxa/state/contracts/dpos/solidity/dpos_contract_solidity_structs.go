...
//		 2. To get implementation bytecode(EblaDposImplBytecode)
//			a) run `solc --bin-runtime --overwrite --optimize dpos_contract_impl.sol --output-dir .`
//			b) Copy bytecode from `DposDummyImpl.bin-runtime` file to `var EblaDposImplBytecode` variable in `dpos_contract_solidity_structs.go` file.
